...
module.exports = function(assemble){
    return function(options, cb) {
        assemble.engine('.hbs').render(options.fn(), options.data.root, function(err, html) {
          ...
          var docsContent = '';
          if(jsonContent.docs) {
              docsContent = iterateJSONContentByRef(assemble, jsonContent.docs);
          }
          var tmpl = template({
              preview: html,
              ...
              code: {
                  ...
                  json: hljs.highlightAuto(beautifyJS(JSON.stringify(jsonContent.data||jsonContent)), ['json']).value,
...
function iterateJSONContentByRef(assemble, content) {
    var docsString = JSON.stringify(content);
    return docsString.replace(/\1([\'\"]+ref:)+\2([a-zA-Z0-9\/-_]+)\"/gm, function(source, prefix, value) {
        var content = getJSONContent(assemble, value);
        return iterateJSONContentByRef(assemble, content.docs || {});
    });
}
